<x-app-layout>
    ...
    <div class="space-y-6">
        <x-card title="Penerbit" :actions="[['label' => 'Tambah Penerbit', 'href' => route('publishers.create'), 'variant' => 'primary']]">
            <form action="{{ route('publishers.index') }}" method="GET" class="mb-4">
                <div class="flex flex-col sm:flex-row gap-2">
                    <x-text-input type="text" name="search" :value="request('search')" placeholder="Cari penerbit..." />
                                ...
                                <tr class="bg-white transition-colors hover:bg-cream-50 dark:bg-slate-900 dark:hover:bg-slate-800/60">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <div class="flex items-center gap-1">
                                            <a href="{{ route('publishers.edit', $publisher) }}" class="inline-flex items-center gap-1 rounded-lg bg-primary-50 px-3 py-1.5 text-xs font-medium text-primary-700 transition-colors hover:bg-primary-100 dark:bg-primary-500/10 dark:text-primary-400 dark:hover:bg-primary-500/20">
                                                Edit
                                            </a>
                                            <form action="{{ route('publishers.destroy', $publisher) }}" method="POST" data-confirm="Yakin ingin menghapus penerbit ini?">
                                                @csrf
                                                @method('DELETE')
            ...
            @else
                <x-empty-state title="Belum ada data penerbit" description="Mulai tambahkan penerbit baru." icon="building-office" />
            @endif
        </x-card>
    </div>
</x-app-layout>
