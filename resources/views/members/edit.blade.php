            ...
            <form action="{{ route('members.update', $member) }}" method="POST">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div class="space-y-1">
                        ...
                        <x-text-input type="text" name="nis" :value="old('nis', $member->nis)" required />
                        <x-input-error :messages="$errors->get('nis')" />
                    </div>

                    <div class="space-y-1">
                        <x-input-label value="Nama Lengkap" />
                        <x-text-input type="text" name="name" :value="old('name', $member->name)" required />
                        <x-input-error :messages="$errors->get('name')" />
                    </div>

                    <div class="space-y-1">
                        <x-input-label value="Email (opsional, auto-generate jika kosong)" />
                        <x-text-input type="email" name="email" :value="old('email', $member->user->email ?? '')" />
